...
def создать_выборки():
  if not os.path.exists('инструменты/квартиры'):
      os.makedirs('инструменты/квартиры')
  assert os.path.exists('moscow.csv'), 'Загрузите базу \'квартиры\'.'
  df = pd.read_csv('moscow.csv', sep=";") #Загружаем данные в data frame
  df = df.iloc[::2,:] #Выбираем нечётные строки, в чётных строках в исходном фрейме пустые строки для комментариев
  data = df.values #Вытаскиваем данные в numpy array
  oneRoomMask = [getRoomsCount(d, 30) == 1 for d in data] #Делаем маску однокомнатных квартир, принцип (getRoomsCount(d, 30) == 1)
  ...
  pickle.dump(yScaler, open('инструменты/квартиры/yscaler.pkl','wb'))
  pickle.dump(xScaler, open('инструменты/квартиры/xscaler.pkl','wb'))
...
def тест_модели(нейронка, *данные):
  xScaler = pickle.load(open('инструменты/квартиры/xscaler.pkl','rb'))
  yScaler = pickle.load(open('инструменты/квартиры/yscaler.pkl','rb'))
